#!/usr/bin/env bash
# CATALOG-STORAGE-PATH-AUTH-1: proves that a database built from the
...
#
# Modelled on 066_catalog_write_authorization_catalog_equivalence.sh; its
# generic machinery (schema resolution, the non-vacuous compare(), every
# catalog section) is carried over unchanged.
#
...
#
#   * path A creates books fresh, inherits the platform's ambient
#     table-level grants, then issues the reset and column grants;
#   * path B starts from the Patch 7 ACL (column INSERT on 17 books
#     columns, cover_path and file_path included) and must arrive at the
#     same place. A migration that only granted the 15 columns without the
#     reset would leave the two path columns insertable on path B only.
...
# -- including a row that ALREADY carries a forged path and a legacy row
# with non-canonical paths -- and proves (a) it rewrites no row, (b) no
# policy and no ACL outside public.books changes (public.bundles
# included), and (c) the intended behaviour change, measured with
# rollback-only probes: an author's direct insert naming a path column
# (null, own canonical, another author's) succeeds before and is refused
# after, while the pathless draft insert, the series unlink, deletes and
# the service_role insert work throughout.
#
# Usage, from the repo root, against a cluster where the connecting
# user may create and drop databases (PostgreSQL 17 or newer):
#
#   ./supabase/tests/067_catalog_storage_path_authorization_catalog_equivalence.sh
#
...
# written against: origin/staging at the time, the merge of PR #28.
BASE_REF="${BASE_REF:-f15a36d9f9c93f7d4f0c9399c48525b224db68d9}"
MIGRATION="${MIGRATION:-supabase/migrations/20260924141734_catalog_storage_path_authorization.sql}"
...
MARKER="${MARKER:-CATALOG-STORAGE-PATH-AUTH-1}"
...
SUFFIX="$$"
DB_A="librum_storage_path_a_${SUFFIX}"
DB_B="librum_storage_path_b_${SUFFIX}"
DB_C="librum_storage_path_c_${SUFFIX}"
WORKDIR="$(mktemp -d)"
...
# The base must actually be the OPEN state -- the Patch 7 books INSERT
# grant that still lists both path columns -- or "the hole is closed" is
# being proved against a database that never had it.
if ! tr -s ' \n' ' ' < "$WORKDIR/base_schema.sql" | grep -q 'series_position, price_all, cover_path, file_path ) on public.books to authenticated;'; then
  echo "FAIL: ${BASE_REF}:${SCHEMA_PATH} does not grant authenticated INSERT on the path columns -- wrong base" >&2
  exit 1
fi
...
for db in "$DB_A" "$DB_B"; do
  ...
  PRESENT="$(psql -X -q -t -A -d "$db" -v ON_ERROR_STOP=1 -c "
    select (
      -- the two path columns are unwritable by every client-facing role
      not has_column_privilege('authenticated', 'public.books', 'file_path', 'INSERT')
      and not has_column_privilege('authenticated', 'public.books', 'cover_path', 'INSERT')
      and not has_column_privilege('authenticated', 'public.books', 'file_path', 'UPDATE')
      and not has_column_privilege('authenticated', 'public.books', 'cover_path', 'UPDATE')
      ...
      and not has_table_privilege('authenticated', 'public.books', 'INSERT')
      and not has_table_privilege('authenticated', 'public.books', 'UPDATE')
      -- the pathless draft columns and the series link stay writable
      and has_column_privilege('authenticated', 'public.books', 'title', 'INSERT')
      ...
      -- SELECT and author DELETE kept; anon read-only
      ...
      -- bundles keeps its Patch 7 draft-create grant
      ...
      and has_column_privilege('service_role', 'public.books', 'file_path', 'INSERT')
      and has_column_privilege('service_role', 'public.books', 'cover_path', 'INSERT')
  ...
  if [ -s "$WORKDIR/present.err" ] || [ "$PRESENT" != "true" ]; then
    echo "FAIL: $db does not carry the CATALOG-STORAGE-PATH-AUTH-1 books ACL" >&2
    tail -n 5 "$WORKDIR/present.err" >&2 || true
    FAILURES=$((FAILURES + 1))
  else
    echo "  ok [carries_change:$db]: file_path/cover_path unwritable by public/anon/authenticated, pathless draft columns and series link kept, bundles unchanged, service_role intact"
  fi
done
...
insert into auth.users (id, email, email_confirmed_at, raw_user_meta_data) values
  ('e0670f00-0000-0000-0000-00000000000a', 'p067-harness-a@test', now(), '{"role":"author","display_name":"P067 Harness A"}'),
  ('e0670f00-0000-0000-0000-00000000000b', 'p067-harness-b@test', now(), '{"role":"author","display_name":"P067 Harness B"}');

update public.profiles set role = 'author'
  where id in ('e0670f00-0000-0000-0000-00000000000a', 'e0670f00-0000-0000-0000-00000000000b');

insert into public.series (id, author_id, title) values
  ('e0670f03-0000-0000-0000-00000000000a', 'e0670f00-0000-0000-0000-00000000000a', 'P067 Harness Series');

-- Canonical rows, a legacy non-canonical row, a row with null paths and a
-- row that ALREADY carries a forged (other author's) manuscript path: the
-- migration must leave every one of them exactly as it is.
insert into public.books (id, author_id, title, status, price_all, price_cents, published_at, series_id, series_position, cover_path, file_path) values
  ('e0670f01-0000-0000-0000-000000000001', 'e0670f00-0000-0000-0000-00000000000a', 'P067H draft paid',   'draft',     199,  0,   null,                   'e0670f03-0000-0000-0000-00000000000a', 1,
   'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-000000000001-cover.png', 'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-000000000001.epub'),
  ('e0670f01-0000-0000-0000-000000000002', 'e0670f00-0000-0000-0000-00000000000a', 'P067H published',    'published', 199,  0,   '2026-09-23T21:16:26Z', 'e0670f03-0000-0000-0000-00000000000a', 2,
   'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-000000000002-cover.jpg', 'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-000000000002.epub'),
  ('e0670f01-0000-0000-0000-000000000003', 'e0670f00-0000-0000-0000-00000000000a', 'P067H legacy paths', 'published', null, 799, '2026-08-01T00:00:00Z', null, null,
   'covers/legacy.JPG', 'legacy/manuscript.epub'),
  ('e0670f01-0000-0000-0000-000000000004', 'e0670f00-0000-0000-0000-00000000000a', 'P067H null paths',   'draft',     0,    0,   null,                   null, null,
   null, null),
  ('e0670f01-0000-0000-0000-000000000005', 'e0670f00-0000-0000-0000-00000000000a', 'P067H forged before','draft',     0,    0,   null,                   null, null,
   'e0670f00-0000-0000-0000-00000000000b/e0670f01-0000-0000-0000-000000000006-cover.png', 'e0670f00-0000-0000-0000-00000000000b/e0670f01-0000-0000-0000-000000000006.epub'),
  ('e0670f01-0000-0000-0000-000000000006', 'e0670f00-0000-0000-0000-00000000000b', 'P067H victim',       'draft',     0,    0,   null,                   null, null,
   'e0670f00-0000-0000-0000-00000000000b/e0670f01-0000-0000-0000-000000000006-cover.png', 'e0670f00-0000-0000-0000-00000000000b/e0670f01-0000-0000-0000-000000000006.epub');

insert into public.bundles (id, author_id, title, status, price_all, price_cents) values
  ('e0670f02-0000-0000-0000-000000000001', 'e0670f00-0000-0000-0000-00000000000a', 'P067H published bundle', 'published', 0,   0);

insert into public.bundle_books (bundle_id, book_id) values
  ('e0670f02-0000-0000-0000-000000000001', 'e0670f01-0000-0000-0000-000000000002');
FIXTURES
...
# books, every column ACL except books (bundles included), every function
# and schema ACL, and every policy of every table (books included).
...
select 'rel:' || n.nspname || '.' || c.relname || '=' || coalesce(c.relacl::text, '<default>')
 ...
 where n.nspname not in ('pg_catalog', 'information_schema') and n.nspname not like 'pg\_%'
   ...
   and not (n.nspname = 'public' and c.relname = 'books')
union all
select 'col:' || c.relname || '.' || a.attname || '=' || a.attacl::text
  from pg_catalog.pg_attribute a join pg_catalog.pg_class c on c.oid = a.attrelid
  join pg_catalog.pg_namespace n on n.oid = c.relnamespace
 where n.nspname = 'public' and a.attacl is not null and c.relname <> 'books'
union all
...
begin
  ...
  perform set_config('request.jwt.claim.sub',
    case when p_role = 'authenticated' then 'e0670f00-0000-0000-0000-00000000000a' else '' end, true);
...
select pg_temp.probe('insert_forged_file', 'authenticated', $q$insert into public.books (author_id, title, file_path) values ('e0670f00-0000-0000-0000-00000000000a', 'forged', 'e0670f00-0000-0000-0000-00000000000b/e0670f01-0000-0000-0000-000000000006.epub')$q$);
select pg_temp.probe('insert_forged_cover', 'authenticated', $q$insert into public.books (author_id, title, cover_path) values ('e0670f00-0000-0000-0000-00000000000a', 'forged', 'e0670f00-0000-0000-0000-00000000000b/e0670f01-0000-0000-0000-000000000006-cover.png')$q$);
select pg_temp.probe('insert_null_file', 'authenticated', $q$insert into public.books (author_id, title, file_path) values ('e0670f00-0000-0000-0000-00000000000a', 'null file', null)$q$);
select pg_temp.probe('insert_null_cover', 'authenticated', $q$insert into public.books (author_id, title, cover_path) values ('e0670f00-0000-0000-0000-00000000000a', 'null cover', null)$q$);
select pg_temp.probe('insert_own_canonical', 'authenticated', $q$insert into public.books (id, author_id, title, cover_path, file_path) values ('e0670f01-0000-0000-0000-0000000000c1', 'e0670f00-0000-0000-0000-00000000000a', 'own', 'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-0000000000c1-cover.png', 'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-0000000000c1.epub')$q$);
select pg_temp.probe('insert_pathless_draft', 'authenticated', $q$insert into public.books (id, author_id, title, subtitle, description, keywords, isbn, language, publisher, edition, original_publication_date, genre, series_id, series_position, price_all) values (gen_random_uuid(), 'e0670f00-0000-0000-0000-00000000000a', 'draft', null, 'd', '', null, 'sq', null, null, null, 'Fiction', null, null, 199)$q$);
select pg_temp.probe('update_file_path', 'authenticated', $q$update public.books set file_path = 'e0670f00-0000-0000-0000-00000000000b/e0670f01-0000-0000-0000-000000000006.epub' where id = 'e0670f01-0000-0000-0000-000000000001'$q$);
select pg_temp.probe('update_series_unlink', 'authenticated', $q$update public.books set series_id = null, series_position = null where series_id = 'e0670f03-0000-0000-0000-00000000000a' and author_id = 'e0670f00-0000-0000-0000-00000000000a'$q$);
select pg_temp.probe('bundle_insert_draft', 'authenticated', $q$insert into public.bundles (author_id, title, description, price_all) values ('e0670f00-0000-0000-0000-00000000000a', 'draft', '', 299)$q$);
select pg_temp.probe('service_trusted_insert', 'service_role', $q$insert into public.books (id, author_id, title, price_all, cover_path, file_path) values ('e0670f01-0000-0000-0000-0000000000e1', 'e0670f00-0000-0000-0000-00000000000a', 'trusted', 199, 'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-0000000000e1-cover.png', 'e0670f00-0000-0000-0000-00000000000a/e0670f01-0000-0000-0000-0000000000e1.epub')$q$);
select pg_temp.probe('delete_own_book', 'authenticated', $q$delete from public.books where id = 'e0670f01-0000-0000-0000-000000000004'$q$);
select pg_temp.probe('anon_insert', 'anon', $q$insert into public.books (author_id, title) values ('e0670f00-0000-0000-0000-00000000000a', 'anon')$q$);
rollback;
PROBE
...
if [ "$STEP_RC" -ne 0 ] || [ -s "$WORKDIR/before.err" ] || [ -s "$WORKDIR/after.err" ] \
  ...
  echo "  ok [populated_migration]: the six books (canonical, legacy non-canonical, null-path, an already-forged row and its victim), the bundle and its membership are byte-identical after the migration; the probes left nothing behind"
fi
...
if [ "$UNRELATED_ROWS" -lt 50 ] || ! grep -q '^pol:public.books.' "$WORKDIR/unrelated_before.out"; then
  ...
  echo "FAIL [no_unrelated_drift]: something outside the books table and column ACLs changed" >&2
  diff "$WORKDIR/unrelated_before.out" "$WORKDIR/unrelated_after.out" | head -n 20 >&2
  FAILURES=$((FAILURES + 1))
else
  echo "  ok [no_unrelated_drift]: $UNRELATED_ROWS entries (every other relation/column/function/schema ACL incl. bundles, every policy, RLS flags, books/bundles defaults) identical before and after"
fi

SECTIONS=$((SECTIONS + 1))
PB="$(grep '=' "$WORKDIR/probe_before.out" | tr '\n' ' ' || true)"
PA="$(grep '=' "$WORKDIR/probe_after.out" | tr '\n' ' ' || true)"
EXPECT_BEFORE="insert_forged_file=OK:1 insert_forged_cover=OK:1 insert_null_file=OK:1 insert_null_cover=OK:1 insert_own_canonical=OK:1 insert_pathless_draft=OK:1 update_file_path=42501 update_series_unlink=OK:2 bundle_insert_draft=OK:1 service_trusted_insert=OK:1 delete_own_book=OK:1 anon_insert=42501 "
EXPECT_AFTER="insert_forged_file=42501 insert_forged_cover=42501 insert_null_file=42501 insert_null_cover=42501 insert_own_canonical=42501 insert_pathless_draft=OK:1 update_file_path=42501 update_series_unlink=OK:2 bundle_insert_draft=OK:1 service_trusted_insert=OK:1 delete_own_book=OK:1 anon_insert=42501 "
if [ "$PB" != "$EXPECT_BEFORE" ]; then
  ...
  echo "  ok [rollout_effect]: an author's direct INSERT naming file_path or cover_path (another author's, own canonical, or null) succeeds BEFORE and is refused AFTER; the pathless draft, the series unlink, bundle drafts, deletes and the service_role insert work throughout; path UPDATE and anon INSERT are refused throughout"
...
if [ "$FAILURES" -ne 0 ]; then
  echo "FAIL: 067_catalog_storage_path_authorization_catalog_equivalence.sh -- $FAILURES of $SECTIONS comparisons failed" >&2
  exit 1
fi

echo "PASS: 067_catalog_storage_path_authorization_catalog_equivalence.sh -- $SECTIONS catalog sections identical between $PATCHED_LABEL and ${BASE_REF:0:9} + migration, none empty"
