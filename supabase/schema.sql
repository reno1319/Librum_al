...
create table public.book_checkout_intents (
  ...
  royalty_rate_bps integer
    check (royalty_rate_bps is null or (royalty_rate_bps >= 0 and royalty_rate_bps <= 10000)),
  -- STALE-CHECKOUT-1: the terminal, NON-FINANCIAL state. An economic
  -- quote is never rewritten -- it is superseded, and a fresh quote is
  -- minted that re-reads the current book, publication state, price,
  -- currency, regime and discount.
  --
  -- superseded and completed/fulfilled are ORTHOGONAL: (superseded,
  -- fulfilled) is a legal, expected, auditable state meaning "we gave up
  -- on this quote and then its money arrived anyway". That is why no
  -- CHECK below mentions completed_at or fulfilled_at -- "supersession
  -- happened while incomplete" is a TRANSITION fact, invisible to a
  -- CHECK, and lives in the transition trigger instead.
  --
  -- This vocabulary is deliberately DIFFERENT from
  -- pok_book_checkout_orders.retired_reason and shares no value with it:
  -- one value could not satisfy both CHECKs, and a mapping-specific
  -- reason written onto an intent would be a false record.
  superseded_at timestamptz,
  superseded_reason text,

  check (expires_at > created_at),
  -- STALE-CHECKOUT-1: every constraint this repair introduces is named
  -- EXPLICITLY, here and in the migration, so that a database built from
  -- this file and one built from base + migration agree on constraint
  -- NAMES and not merely on definitions. An anonymous CHECK whose
  -- expression spans more than one column is auto-named
  -- <table>_check<n>, and n depends on declaration order, which the two
  -- build paths do not share.
  constraint book_checkout_intents_superseded_pairing_check
    check ((superseded_at is null) = (superseded_reason is null)),
  constraint book_checkout_intents_superseded_reason_check
...
-- STALE-CHECKOUT-1: supports the temporary supersession guard in
...
-- STALE-CHECKOUT-1: the transition trigger, renamed from
-- enforce_book_checkout_intents_financial_facts_immutability() because
-- that name is now wrong -- it enforces transitions, not only field
-- immutability. Bound BEFORE INSERT OR UPDATE, and it branches on tg_op
-- FIRST, because referencing OLD in an INSERT trigger raises at runtime.
--
-- It still protects only regime/currency/royalty_rate_bps among the
-- original columns (STRIPE-CUTOVER-1C, migration 056); completed_at/
-- fulfilled_at/reconciliation_reason remain freely updatable, which is
-- exactly what lets a late payment fulfil an already-superseded intent.
...
create table public.pok_book_checkout_orders (
  ...
  state text not null default 'creating'
    constraint pok_book_checkout_orders_state_check
    check (state in ('creating', 'ready', 'needs_reconciliation', 'retired')),
  last_error_code text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  -- STALE-CHECKOUT-1: the window Librum REQUESTED, never what POK
  -- enforced. It may justify retiring exactly one thing: an attempt with
  -- no recorded provider_order_id, whose checkout URL was therefore
  -- never disclosed (_self.confirmUrl is only ever surfaced as
  -- startPokCheckout's return value). An attempt WITH an order id is
  -- retired only after an authenticated POK retrieval says so.
  ...
  -- note on book_checkout_intents above.
  constraint pok_book_checkout_orders_retired_pairing_check
    check ((retired_at is null) = (retired_reason is null)),
  -- State and its timestamp are one fact, so they cannot disagree.
  constraint pok_book_checkout_orders_retired_state_check
    check ((state = 'retired') = (retired_at is not null)),
  -- The MAPPING's vocabulary. No value is shared with
  -- book_checkout_intents.superseded_reason.
  constraint pok_book_checkout_orders_retired_reason_check
