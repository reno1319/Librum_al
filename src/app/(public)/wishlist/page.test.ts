...
// BookCard (found by the exhaustive schema-sensitive route audit, not
...
vi.mock("@/app/(public)/books/[id]/actions", () => ({ removeFromWishlist: vi.fn() }));

const { default: WishlistPage } = await import("./page");

describe("WishlistPage: maintenance-mode gate", () => {
  beforeEach(() => {
  ...
  it("maintenance mode off (unset) preserves existing behavior -- the page still reaches Supabase", async () => {
    vi.unstubAllEnvs();
    await expect(WishlistPage()).rejects.toBe(SENTINEL);
